...
public class RemoteManager {
    ...
    public var voiceSessionStarted: ((Int)->Void)?
    public var voiceSessionEnded: ((Int)->Void)?
    ...
    private func handleVoiceBegin() -> Bool {
        guard let sessionID = CommandNetwork.RemoteVoiceMessage.extractBeginSessionID(from: data) else {
            return false
        }

        logger?.debugLog(logPrefix + "voice session started: \(sessionID)")
        data.removeAll()
        voiceSessionStarted?(sessionID)
        return true
    }

    private func handleVoiceEnd() -> Bool {
        guard let sessionID = CommandNetwork.RemoteVoiceMessage.extractEndSessionID(from: data) else {
            return false
        }

        logger?.debugLog(logPrefix + "voice session ended: \(sessionID)")
        data.removeAll()
