...
struct RecentsTabView: View {
    ...
    var body: some View {
        NavigationStack {
                    ...
                    ScrollView {
                        VStack(spacing: 14) {
                            if viewModel.recents.isEmpty {
                                ...
                                ForEach(viewModel.recents) { device in
                                    NavigationLink(destination: TVDetailView(device: device, source: .recents)) {
                                        TVCardView(
                                            device: device,
                                            hasRecent: true,
            ...
            .navigationTitle(L10n.tr("tab.head.recents"))
